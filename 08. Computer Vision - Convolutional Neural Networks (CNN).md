...
4. **Define the Model**
   * Use CNN layers to extract features from images.
   * Add ReLU activations to introduce non-linearity.
   * Use MaxPooling to reduce dimensions.
   * Add Dropout to prevent overfitting.
   * Add Flatten layer to flat 
   * Use Linear (Fully Connected) layers for final classification.
   * For binary classification, the output layer must have 1 neuron (logit output).
