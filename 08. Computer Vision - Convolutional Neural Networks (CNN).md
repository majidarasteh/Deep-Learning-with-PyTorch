...
13. **Number of Filters (Feature Maps)** Start small and increase deeper in the network:

    | Layer | Filters | Example                            |
    | :---- | :------ | :--------------------------------- |
    | Conv1 | 32      | `nn.Conv2d(3, 32, 3, padding=1)`   |
    | Conv2 | 64      | `nn.Conv2d(32, 64, 3, padding=1)`  |
    | Conv3 | 128     | `nn.Conv2d(64, 128, 3, padding=1)` |


15. **Activation Function** Use ReLU for hidden layers, that is fast and stable. Avoid Sigmoid/Tanh inside hidden layers (they cause vanishing gradients).
16. **Optimizer**: For most image classification tasks:
    * Adam → adaptive, fast convergence
    * SGD + Momentum → slower but sometimes better generalization
...
16. **Loss Function**: For binary classification, always use:
    
    ```python
        criterion = nn.BCEWithLogitsLoss()
    ```
    * It combines Sigmoid + Binary Cross Entropy internally.
    * Output layer should have 1 logit (no activation).

17. **Number of Epochs**
    * Small dataset: **10–30 epochs**
    * Medium dataset (like 2k–10k images): **30–50 epochs**
    * Large dataset: **50+ epochs** or use **early stopping**.

18. **Image Size**: Resize all images to a fixed size (e.g., 128×128 or 224×224). Larger images capture more detail but require more computation. 128×128 is a good balance for cats vs dogs.

    Example:
    ```python
        transforms.Resize((128, 128))
    ```
19. **Data Normalization** Normalize pixel values to help faster convergence:
    ...
    * This maps pixel range from [0,1] → [-1,1].

20. **Convert RGBA → RGB**    
If your image has 4 channels **(RGBA)**, **convert it to 3 channels (RGB)** before processing. The **Alpha (A)** channel represents transparency (opacity) and is usually not useful for classification.
...
```
* Most pretrained models (and CNNs in general) expect **3 channels (RGB)**.
* The A channel doesn’t contain useful information for visual classification.
* Keeping it may cause errors like `OSError: not supported for mode RGBA`.

21. **Use the Right Loss Function for Multiclass Classification**
For multiclass classification, where each image belongs to **exactly one class out of N**, use:

```python
  criterion = nn.CrossEntropyLoss()
```

* Your model output shape should be **[batch_size, num_classes]**
* Your target shape should be **[batch_size]** (with integer labels `0 → num_classes-1`)
* Do not apply `softmax` before passing outputs to `CrossEntropyLoss()` — it applies `log_softmax` internally.

22. **Data Type**: In multiclass problems:

```python
  features → dtype = torch.float32
  targets  → dtype = torch.long   # integer class indices
```
 ...
 * `class_to_idx` matches expected labels
 * Shapes: `[batch, 3, 128, 128]`
 * Output layer `size = 1`
 ...
 * Test accuracy ≥ 70% (baseline)
 * Predictions make sense on random images
