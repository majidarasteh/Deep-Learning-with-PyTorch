...
2. **Our Inputs (Features):** We use several properties of the house as our inputs:
  ...
  2. **Step 2: Model Compilation (Configuring the Learning Process)**
     ...
     * **Loss Function (The Measure of Error):** This function calculates how far the model's predictions are from the true values. The goal of training is to minimize this loss. For regression, common choices are:
       ...
       - `nn.MAELoss()` (Mean Absolute Error): Less sensitive to outliers.
         - $MAE = (1/n) * Σ|y_{true} - y_{pred}|$
...
* **GELU (Gaussian Error Linear Unit)**: $x × Φ(x)$ where $Φ(x)$ is the Gaussian CDF.

**Output Layer Activation:**
* **For regression**, the output layer **typically uses no activation function** (linear activation) or `nn.Identity()`, as we want to predict any continuous value without constraints. However, Common output activation functions for regression:
  - **Linear Activation (No Activation)**: Most common choice for standard regression problems and outputs any real number $(-∞, +∞)$.
  - **ReLU (Rectified Linear Unit)**: When predictions must be non-negative and outputs values from 0 to $+∞$.
  - **Sigmoid**: When predictions need to be bounded between 0 and 1.
  - **Tanh (Hyperbolic Tangent)**: When predictions need to be bounded between -1 and 1.
  - **Exponential Linear Unit (ELU)**: Advanced alternative to ReLU and Outputs values from $-α to +∞$
...
The learning rate is a hyperparameter that controls how much to adjust the model's weights in response to the estimated error each time the weights are updated. It determines the step size during gradient descent optimization. Gradient descent update rule:
...
1. **Too High Learning Rate**
   ...
   * Causes divergence or oscillation
   * Training loss fluctuates wildly
     
     ```
     Loss: 0.5 → 0.8 → 0.3 → 1.2 → (unstable)
     ```
...
2. **Too Low Learning Rate**
   ...
   * Training takes very long
     ...
     ```
     Loss: 0.5 → 0.49 → 0.488 → 0.487 → (very slow improvement)
     ```
...
3. **Optimal Learning Rate**
   ...
   * Stable training process
     ...
     ```
     Loss: 0.5 → 0.4 → 0.3 → 0.2 → (steady decrease)
     ```

**Common Learning Rate Values**
1. **Default starting point**,	Typical range: 0.001, 	**Reason:** Good balance for many problems
2. **Small networks**,	Typical range: 0.01 - 0.1, **Reason:**	Faster convergence possible
3. **Large networks**,	Typical range: 0.0001 - 0.001, **Reason:**	More stability needed
4. **Transfer Learning**,	Typical range: 0.00001 - 0.0001, **Reason:**	Fine-tuning pre-trained weights
5. **SGD with Momentum**,	Typical range: 0.01 - 0.1, **Reason:**	Momentum helps stability
6. **How to Choose Learning Rate:** Start with default values:
   * Adam: 0.001
   * SGD: 0.01

The learning rate is one of the most important hyperparameters to tune, as it directly controls how quickly or slowly your model learns from the data.

## Dropout in Deep Learning
Dropout is a regularization technique for neural networks that prevents overfitting by randomly "dropping out" (setting to zero) a percentage of neurons during training. Dropout stops neurons from becoming overly dependent on specific other neurons, forcing each neuron to be more robust.

* Randomly select neurons to "turn off" with probability `p`
* These neurons don't participate in forward or backward pass
...
class RegressionModelWithDropout(nn.Module):
    def __init__(self, input_size=5, hidden_size=64, output_size=1, dropout_rate=0.3):
...
Another implementation of a deep learning model with droupout
...
1. Small datasets	0.2 - 0.5
2. Large datasets	0.1 - 0.3
3. Input layer	0.1 - 0.2
4. Hidden layers	0.2 - 0.5
5. Output layer	0 (usually no dropout)

Dropout is like training multiple neural networks simultaneously and combining their knowledge. Each training batch sees a different "thinned" version of the network, but during testing, you get the combined wisdom of all these networks working together.

This approach forces the network to learn more robust features that work well in various contexts, rather than relying on specific pathways that might only work well on the training data.
